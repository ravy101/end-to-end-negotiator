...
class CriticModule(nn.Module):
    def __init__(self, query_size, value_size, hidden_size, selection_size, args):
        super(CriticModule, self).__init__()

        self.hidden_size = hidden_size

        self.attn = HierarchicalAttention(query_size, value_size, hidden_size,
            args.critic_dropout, args.critic_init_range)
        ...
        self.dropout = nn.Dropout(args.critic_dropout)

        self.sel_decoders = nn.ModuleList()
        self.sel_decoders.append(nn.Linear(selection_size, 1))

        init_cont(self.sel_encoder, args.critic_init_range)
        init_cont(self.sel_decoders, args.critic_init_range)
    ...
    def forward(self, q, hs, lens, rev_idxs, hid_idxs):
        ...
        outs = torch.cat(outs, 1).view(-1, 1)
...
class CriticModel(nn.Module):
    ...
    def __init__(self, word_dict, item_dict, context_dict, count_dict, args):
        ...
        self.ctx_encoder = MlpContextEncoder(len(self.context_dict), domain.input_length(),
            args.nembed_ctx, args.nhid_ctx, args.critic_dropout, args.critic_init_range, critic_skip_values)

        self.sel_head = CriticModule(
