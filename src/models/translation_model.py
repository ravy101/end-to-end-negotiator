...
class TranslationModel:
    def __init__(self, model_file = 't5_nego2.th', dict_file = "data/message_translation_sample.csv", token_filter = False):
        self.IN_LEN = 100
        self.OUT_LEN = 50
        self.REVERSE = False
        self.FORCE_CPU = True

        self.pretranslated_dict = None
        ...
        self.pretranslated_dict = {}
        if dict_file:
            df_trans = pd.read_csv(dict_file, index_col =0)
            ...
            for ind, row in df_trans.iterrows():
                self.pretranslated_dict[row['sentences'].strip()] = row['Predicted'].strip()
    ...
    def translate(self, text):
        ...
        if text in self.pretranslated_dict.keys():
            cleaned = self.pretranslated_dict[text]
        else:
